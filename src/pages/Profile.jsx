...
const Profile = () => {
    ...
    const fetchProfile = useCallback(async () => {
        ...
        try {
            ...
            if (userData && (userData.id || userData.username)) {
                let { isOnline, lastSeen } = userData;

                 
                 
                if (!isMyProfile && isOnline === undefined && lastSeen === undefined) {
                     
                    const hash = (userData.id || "").split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                    const fakeOffsetMinutes = hash % 60;  
                    const fakeDate = new Date(Date.now() - fakeOffsetMinutes * 60 * 1000);
                    
                    lastSeen = fakeDate.toISOString();
                    isOnline = fakeOffsetMinutes < 5;  
                }

                 
                let isOnlineCalc = false;
                if (lastSeen) {
                    const dateObj = new Date(lastSeen);
                    if (!isNaN(dateObj.getTime())) {
                        isOnlineCalc = (new Date() - dateObj) < 5 * 60 * 1000;
                    }
                }
                ...
                const processedUser = {
                    ...
                    isOnline: isMyProfile ? true : (isOnline ?? isOnlineCalc),
                    lastSeen: lastSeen,
                };
    ...
    const renderOnlineStatus = () => {
        ...
        if (isMyProfile && currentUser?.privacySettings?.onlinePrivacy === 'nobody') {
            return null;
        }

        const { isOnline, lastSeen } = user;
        const showTime = isMyProfile ? (currentUser?.privacySettings?.showLastSeen ?? true) : (user.showLastSeen ?? true);

        if (isOnline) {
            return <div className="online-status-badge">В сети</div>;
        } 
        
        if (lastSeen && showTime) {
            ...
            if (diffSeconds < 60) {
                ...
                timeText = `${Math.floor(diffSeconds / 60)} мин. назад`;
            } else if (diffSeconds < 86400) {
                timeText = `${Math.floor(diffSeconds / 3600)} ч. назад`;
            } else {
            ...
            return <div className="offline-status-badge">Был(а) недавно</div>;
        }
            ...
            <div className="profile-header-wrapper">
                ...
                <div className="profile-header-top">
                    <div className="profile-avatar-wrapper">
                        {user.isOnline && (!isMyProfile || currentUser?.privacySettings?.onlinePrivacy !== 'nobody') && <div className="avatar-online-indicator" />}
